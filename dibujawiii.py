...
class Grafico(QtGui.QWidget):
    def __init__(self):
        ...
        self.arr =[win32api.GetCursorPos() for i in range(3)]
        self.l1=0
        self.l2=0
        self.l3=0
    ...
    def paintEvent(self, e):
        ...
        """if pos[0]>self.arr[0][0]+3 or pos[0]<self.arr[0][0]-3 :
            for i in range(1,len(self.arr)):
                self.arr[i]=self.arr[i-1]
            self.arr[0]=pos
            print self.arr"""
        
        if pos[0]>self.arr[0][0]+3 or pos[0]<self.arr[0][0]-3 or pos[1]>self.arr[0][1]+3 or pos[1]<self.arr[0][1]-3:
            self.arr.append(pos)
        qp = QtGui.QPainter()
        qp.begin(self)
        ...
        try:
            ...
            if len(self.arr)>3:
                for i in range(1,len(self.arr)):
                    ...
                    qp.drawLine(self.arr[i-1][0],self.arr[i-1][1],self.arr[i][0],self.arr[i][1])
...
class MainWindow(QtGui.QMainWindow):
    ...
    def keyPressEvent(self, e):
        if str(e.key())=="32":
            self.Widget.arr =[win32api.GetCursorPos() for i in range(3)]
            print e.key()
