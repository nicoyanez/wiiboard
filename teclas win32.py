...
class MyWindow(QtGui.QWidget):
      ...
      def QueBoton(self,nombre):
            ...
            self.label1.setText("")
            hwnd = win32gui.FindWindowEx(0, 0, 0, None)
            print hwnd
            win32gui.ShowWindow(hwnd, win32con.SW_SHOWNORMAL)
            if nombre=="A":
                  win32api.keybd_event(65, 0)
            if nombre=="B":
                  win32api.keybd_event(66, 0)
            if nombre=="DERECHA":
            ...
            if nombre=="HOME":
                  win32api.keybd_event(win32con.VK_F5, 0)
            if nombre=="-":
                  win32api.keybd_event(win32con.VK_BACK, 0)
            if nombre=="+":
                  win32api.keybd_event(win32con.VK_RETURN, 0)
            if nombre=="1":
                  win32api.keybd_event(win32con.VK_NUMPAD1, 0)
            if nombre=="2":
